...
class OTTableWriter(object):
    ...
    def _gatherG***hForHarfbuzz(self, tables, obj_list, done, objidx, virtual_edges):
        real_links = []
        virtual_links = []
        item_idx = objidx
            ...
            for i, item in enumerate(self.items):
                if getattr(item, "name", None) == "Coverage":
                    ...
                    if id(item) not in done:
                        coverage_idx = item_idx = item._gatherG***hForHarfbuzz(
        ...
        for i, item in enumerate(self.items):
            ...
            if id(item.subWriter) not in done:
                child_idx = item_idx = item.subWriter._gatherG***hForHarfbuzz(
                    tables, obj_list, done, item_idx, virtual_edges
                )
            else:
    ...
    def getAllDataUsingHarfbuzz(self, tableTag):
        """The Whole table is represented as a G***h.
        Assemble g***h data and call Harfbuzz repacker to pack the table.
        ...
        objidx = 0
        virtual_edges = []
        self._gatherG***hForHarfbuzz(tables, obj_list, done, objidx, virtual_edges)
