...
class Glyph(object):
    def __init__(
        ...
        self.g***hicType = g***hicType
        self.imageData = imageData

        # fix self.g***hicType if it is null terminated or too short
        if self.g***hicType is not None:
            if self.g***hicType[-1] == "\0":
                self.g***hicType = self.g***hicType[:-1]
            if len(self.g***hicType) > 4:
                from fontTools import ttLib

                raise ttLib.TTLibError(
                    "Glyph.g***hicType must not be longer than 4 characters."
                )
            elif len(self.g***hicType) < 4:
                # pad with spaces
                self.g***hicType += "    "[: (4 - len(self.g***hicType))]

    def is_reference_type(self):
        """Returns True if this glyph is a reference to another glyph's image data."""
        return self.g***hicType == "dupe" or self.g***hicType == "flip"
