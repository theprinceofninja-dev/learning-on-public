...
def find_parents_and_order(glyphsets, locations, *, discrete_axes=set()):
    ...
    if locations:
        ...
        try:
            from scipy.sparse.csg***h import minimum_spanning_tree

            g***h = [[0] * len(locations) for _ in range(len(locations))]
            axes = set()
            for l in locations:
                axes.update(l.keys())
            axes = sorted(axes)
            ...
            for i, j in itertools.combinations(range(len(locations)), 2):
                ...
                if i_discrete_location != j_discrete_location:
                    continue
                g***h[i][j] = vdiff_hypot2(vectors[i], vectors[j])

            tree = minimum_spanning_tree(g***h, overwrite=True)
            ...
            # Traverse g***h from the base and assign parents
