...
class table_F__e_a_t(DefaultTable.DefaultTable):
    ...
    The ``Feat`` table is used exclusively by the G***hite shaping engine
    to store features and possible settings specified in GDL. G***hite features
    determine what rules are applied to transform a glyph stream.

    Not to be confused with ``feat``, or the OpenType Layout tables
    ``GSUB``/``GPOS``.

    See also https://g***hite.sil.org/g***hite_techAbout#g***hite-font-tables
    """
