...
def setup_module():
    ...
    if w*** is None:
        ...
        config.add_extension('test_array_from_pyobj_ext',
                             sources=['w***module.c', 'fortranobject.c'],
                             define_macros=[])
        """
        d = os.path.dirname(__file__)
        src = [
            util.getpath("tests", "src", "array_from_pyobj", "w***module.c"),
            util.getpath("src", "fortranobject.c"),
            util.getpath("src", "fortranobject.h"),
        ]
        w*** = util.build_module_distutils(src, config_code,
                                           "test_array_from_pyobj_ext")


def flags_info(arr):
    flags = w***.array_attrs(arr)[6]
...
def flags2names(flags):
    ...
    for flagname in [
        ...
        if abs(flags) & getattr(w***, flagname, 0):
...
class Intent:
    def __init__(self, intent_list=[]):
        ...
        for i in intent_list:
            if i == "optional":
                flags |= w***.F2PY_OPTIONAL
            else:
                flags |= getattr(w***, "F2PY_INTENT_" + i.upper())
        self.flags = flags
...
class Type:
    ...
    def _init(self, name):
        ...
        if self.NAME == 'CHARACTER':
            ...
            self.type_num = getattr(w***, 'NPY_STRING')
            self.elsize = 1
            ...
            info = typeinfo[self.NAME]
            self.type_num = getattr(w***, 'NPY_' + self.NAME)
...
class Array:
    ...
    def __init__(self, typ, dims, intent, obj):
        ...
        self.arr = w***.call(typ.type_num,
                             typ.elsize,
                             dims, intent.flags, obj)

        assert isinstance(self.arr, np.ndarray)

        self.arr_attr = w***.array_attrs(self.arr)

        if len(dims) > 1:
            if self.intent.is_intent("c"):
                assert (intent.flags & w***.F2PY_INTENT_C)
                assert not self.arr.flags["FORTRAN"]
                assert self.arr.flags["CONTIGUOUS"]
                assert (not self.arr_attr[6] & w***.FORTRAN)
            else:
                assert (not intent.flags & w***.F2PY_INTENT_C)
                assert self.arr.flags["FORTRAN"]
                assert not self.arr.flags["CONTIGUOUS"]
                assert (self.arr_attr[6] & w***.FORTRAN)
        ...
        self.pyarr_attr = w***.array_attrs(self.pyarr)

        if len(dims) > 1:
            if self.intent.is_intent("c"):
                assert not self.pyarr.flags["FORTRAN"]
                assert self.pyarr.flags["CONTIGUOUS"]
                assert (not self.pyarr_attr[6] & w***.FORTRAN)
            else:
                assert self.pyarr.flags["FORTRAN"]
                assert not self.pyarr.flags["CONTIGUOUS"]
                assert (self.pyarr_attr[6] & w***.FORTRAN)
    ...
    def has_shared_memory(self):
        ...
        if not isinstance(self.obj, np.ndarray):
            return False
        obj_attr = w***.array_attrs(self.obj)
