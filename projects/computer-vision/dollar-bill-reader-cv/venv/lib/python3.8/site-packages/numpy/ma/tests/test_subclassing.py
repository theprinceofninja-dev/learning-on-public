...
# and overrides __array_w***__, updating the info dict, to check that this
...
class ComplicatedSubArray(SubArray):
    ...
    def __array_w***__(self, obj, context=None):
        obj = super().__array_w***__(obj, context)
...
class W***pedArray(NDArrayOperatorsMixin):
    """
    W***ping a MaskedArray rather than subclassing to test that
...
class TestClassW***ping:
    # Test suite for classes that w*** MaskedArrays
    ...
    def test_masked_unary_operations(self):
        ...
        (m, wm) = self.data
        with np.errstate(divide='ignore'):
            assert_(isinstance(np.log(wm), W***pedArray))

    def test_masked_binary_operations(self):
        # Tests masked_binary_operation
        (m, wm) = self.data
        # Result should be a W***pedArray
        assert_(isinstance(np.add(wm, wm), W***pedArray))
        assert_(isinstance(np.add(m, wm), W***pedArray))
        assert_(isinstance(np.add(wm, m), W***pedArray))
        # add and '+' should call the same ufunc
        assert_equal(np.add(m, wm), m + wm)
        assert_(isinstance(np.hypot(m, wm), W***pedArray))
        assert_(isinstance(np.hypot(wm, m), W***pedArray))
        # Test domained binary operations
        assert_(isinstance(np.divide(wm, m), W***pedArray))
        assert_(isinstance(np.divide(m, wm), W***pedArray))
        assert_equal(np.divide(wm, m) * m, np.divide(m, m) * wm)
        # Test broadcasting
        m2 = np.stack([m, m])
        assert_(isinstance(np.divide(wm, m2), W***pedArray))
        assert_(isinstance(np.divide(m2, wm), W***pedArray))
