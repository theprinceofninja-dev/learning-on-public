...
Here is a skeleton of a new w***per function (13Dec2001):
...
from .auxfuncs import (
    ...
    isdummyroutine, isexternal, isfunction, isfunction_w***, isint1,
    ...
    issubroutine_w***, isthreadsafe, isunsigned, isunsigned_char,
    isunsigned_chararray, isunsigned_long_long,
    isunsigned_long_longarray, isunsigned_short, isunsigned_shortarray,
    l_and, l_not, l_or, outmess, replace, stripcomma, requiresf90w***per
)
...
defmod_rules = [
    {'body': '/*eof body*/',
     ...
     'initf2pyw***hooks': '/*eof initf2pyw***hooks*/',
...
""",
    ...
    'initf2pyw***hooks': '#initf2pyw***hook#',
    ...
    'latexdoc': ['\\subsection{W***per function \\texttt{#texname#}}\n',
                 """
...
rout_rules = [
    {  # Init
        ...
        '_check': l_and(issubroutine, l_not(issubroutine_w***)),
    }, {  # W***ped function
        'functype': 'void',
        ...
        'initf2pyw***hook': {l_not(l_or(ismoduleroutine, isdummyroutine)): '''
    {
    ...
    '''},
        ...
        'callfortranroutine': [
            {debugcapi: [
                """    fprintf(stderr,\"debug-capi:Fortran subroutine `f2pyw***#name_lower#(#callfortran#)\'\\n\");"""]},
            {hasexternals: """\
    if (#setjmpbuf#) {
        f2py_success = 0;
    } else {"""},
...
"""}
        ...
        '_check': l_and(isfunction, l_not(isfunction_w***))
    }, {  # Scalar function
...
'''},
        ...
        '_check': l_and(isfunction, l_not(isstringfunction), l_not(isfunction_w***))
    }, {  # String function # in use for --no-w***
...
""",
        ...
        '_check':l_and(isstringfunction, l_not(isfunction_w***))  # ???obsolete
    },
    {  # Debugging
...
def buildmodule(m, um):
    ...
    for n in m['interfaced']:
        ...
        if 'entry' in nb:
            for k, a in nb['entry'].items():
                ...
                nb_list.append(nb1)
        for nb in nb_list:
            # requiresf90w***per must be called before buildapi as it
            # rewrites assumed shape arrays as automatic arrays.
            isf90 = requiresf90w***per(nb)
            # options is in scope here
            if options['emptygen']:
                b_path = options['buildpath']
                m_name = vrd['modulename']
                outmess('    Generating possibly empty w***pers"\n')
                Path(f"{b_path}/{vrd['coutput']}").touch()
                if isf90:
                    # f77 + f90 w***pers
                    outmess(f'    Maybe empty "{m_name}-f2pyw***pers2.f90"\n')
                    Path(f'{b_path}/{m_name}-f2pyw***pers2.f90').touch()
                    outmess(f'    Maybe empty "{m_name}-f2pyw***pers.f"\n')
                    Path(f'{b_path}/{m_name}-f2pyw***pers.f').touch()
                else:
                    # only f77 w***pers
                    outmess(f'    Maybe empty "{m_name}-f2pyw***pers.f"\n')
                    Path(f'{b_path}/{m_name}-f2pyw***pers.f').touch()
    ...
    cr, w*** = common_rules.buildhooks(m)
    if w***:
        funcw***pers.append(w***)
    ...
    mr, w*** = f90mod_rules.buildhooks(m)
    if w***:
        funcw***pers2.append(w***)
    ...
    if options['dolatexdoc']:
        ...
        outmess('    Documentation is saved to file "%s/%smodule.tex"\n' %
                (options['buildpath'], vrd['modulename']))
    if funcw***pers:
        wn = os.path.join(options['buildpath'], vrd['f2py_w***per_output'])
        ret['fsrc'] = wn
        with open(wn, 'w') as f:
            f.write('C     -*- fortran -*-\n')
            f.write(
            ...
            f.write(
                'C     It contains Fortran 77 w***pers to fortran functions.\n')
            lines = []
            for l in ('\n\n'.join(funcw***pers) + '\n').split('\n'):
                if 0 <= l.find('!') < 66:
            ...
            f.write(lines)
        outmess('    Fortran 77 w***pers are saved to "%s"\n' % (wn))
    if funcw***pers2:
        wn = os.path.join(
            options['buildpath'], '%s-f2pyw***pers2.f90' % (vrd['modulename']))
        ret['fsrc'] = wn
        with open(wn, 'w') as f:
            f.write('!     -*- f90 -*-\n')
            f.write(
            ...
            f.write(
                '!     It contains Fortran 90 w***pers to fortran functions.\n')
            lines = []
            for l in ('\n\n'.join(funcw***pers2) + '\n').split('\n'):
                if 0 <= l.find('!') < 72:
                    # don't split comment lines
            ...
            lines = ''.join(lines).replace('\n     &\n', '\n')
            f.write(lines)
        outmess('    Fortran 90 w***pers are saved to "%s"\n' % (wn))
    return ret
...
def buildapi(rout):
    rout, w*** = func2subr.assubr(rout)
    args, depargs = getargs2(rout)
    capi_maps.depargs = depargs
    var = rout['vars']

    if ismoduleroutine(rout):
        outmess('            Constructing w***per function "%s.%s"...\n' %
                (rout['modulename'], rout['name']))
    else:
        outmess('        Constructing w***per function "%s"...\n' % (rout['name']))
    # Routine
