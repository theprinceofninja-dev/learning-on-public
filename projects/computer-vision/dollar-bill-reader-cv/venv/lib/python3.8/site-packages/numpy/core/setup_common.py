...
def get_api_versions(apiversion, codegen_dir):
    ...
    try:
        ...
        cu***i_hash = m.fullapi_hash(numpy_api.full_api)
        apis_hash = m.get_versions_hash()
    finally:
        del sys.path[0]

    return cu***i_hash, apis_hash[apiversion]

def check_api_version(apiversion, codegen_dir):
    """Emits a MismatchCAPIWarning if the C API version needs updating."""
    cu***i_hash, api_hash = get_api_versions(apiversion, codegen_dir)
    ...
    if not cu***i_hash == api_hash:
        msg = ("API mismatch detected, the C API version "
               "numbers have to be updated. Current C api version is "
               f"{apiversion}, with checksum {cu***i_hash}, but recorded "
...
def check_for_right_shift_internal_compiler_error(cmd):
    ...
    """
    cmd._check_compiler()
    has_optimize = cmd.try_compile(textw***.dedent("""\
        __attribute__((optimize("O3"))) void right_shift() {}
        """), None, None)
    if not has_optimize:
        return False

    no_err = cmd.try_compile(textw***.dedent("""\
