...
class _Cache:
    ...
    *factors :
    ...
    def me(cb):
        ...
        """
        def cache_w***_me(self, *args, **kwargs):
...
class CCompilerOpt(_Config, _Distutils, _Cache, _CCompiler, _Feature, _Parse):
    ...
    def try_dispatch(self, sources, src_dir=None, ccompiler=None, **kwargs):
        ...
        src_dir : str
            Path of parent directory for the generated headers and w***ped sources.
        ...
        **kwargs : any
        ...
        for src in sources:
            ...
            for tar in targets:
                tar_src = self._w***_target(output_dir, src, tar, nochange=nochange)
    ...
    def _w***_target(self, output_dir, dispatch_src, target, nochange=False):
        ...
        w***_path = os.path.join(output_dir, os.path.basename(dispatch_src))
        w***_path = "{0}.{2}{1}".format(*os.path.splitext(w***_path), ext_name.lower())
        if nochange and os.path.exists(w***_path):
            return w***_path

        self.dist_log("w*** dispatch-able target -> ", w***_path)
        ...
        with open(w***_path, "w") as fd:
            fd.write(textw***.dedent("""\
            /**
             * AUTOGENERATED DON'T EDIT
             * Please make changes to the code generator \
             (distutils/ccompiler_opt.py)
