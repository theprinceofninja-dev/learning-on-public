...
class TestOut:
    ...
    def test_out_w***_subok(self):
        class ArrayW***(np.ndarray):
            __array_priority__ = 10

            def __new__(cls, arr):
                return np.asarray(arr).view(cls).copy()

            def __array_w***__(self, arr, context):
...
class TestSpecialMethods:
    ...
    def test_w***_and_prepare_out(self):
        # Calling convention for out should not affect how special methods are
        # called

        class StoreArrayPrepareW***(np.ndarray):
            _w***_args = None
            _prepare_args = None
            def __new__(cls):
                return np.zeros(()).view(cls)
            def __array_w***__(self, obj, context):
                self._w***_args = context[1]
                return obj
            ...
            def args(self):
                ...
                return (self._prepare_args, self._w***_args)
            def __repr__(self):
                return "a"  # for short test output

        def do_test(f_call, f_expected):
            a = StoreArrayPrepareW***()
            f_call(a)
            p, w = a.args
    ...
    def test_w***_with_iterable(self):
        # test fix for bug #1026:

        class with_w***(np.ndarray):
            __array_priority__ = 10
    ...
    def test_failing_out_w***(self):

        singleton = np.array([1.0])

        class Ok(np.ndarray):
            def __array_w***__(self, obj):
                return singleton
    ...
    def test_none_w***(self):
        ...
        class A:
            ...
            def __array_w***__(self, arr, context=None):
                return None
