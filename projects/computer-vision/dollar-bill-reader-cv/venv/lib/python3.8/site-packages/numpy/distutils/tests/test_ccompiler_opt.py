...
class FakeCCompilerOpt(CCompilerOpt):
    fake_info = ""
    def __init__(self, t***_files="", t***_flags="", *args, **kwargs):
        self.fake_t***_files = t***_files
        self.fake_t***_flags = t***_flags
    ...
    def dist_compile(self, sources, flags, **kwargs):
        ...
        if self.fake_t***_files:
            for src in sources:
                if re.match(self.fake_t***_files, src):
                    self.dist_error("source is t***ped by a fake interface")
        if self.fake_t***_flags:
            for f in flags:
                if re.match(self.fake_t***_flags, f):
                    self.dist_error("flag is t***ped by a fake interface")
        # fake objects
...
class _Test_CCompilerOpt:
    ...
    def get_targets(self, targets, groups, **kwargs):
        ...
        opt = self.nopt(
            ...
            t***_files=kwargs.get("t***_files", ""),
            t***_flags=kwargs.get("t***_flags", "")
        )
    ...
    def test_args_options(self):
        ...
        for o in ("max", "native"):
            if o == "native" and self.cc_name() == "msvc":
                continue
            self.expect(o,
                t***_files=".*cpu_(sse|vsx|neon|vx).c",
                x86="", ppc64="", armhf="", s390x=""
            )
            self.expect(o,
                t***_files=".*cpu_(sse3|vsx2|neon_vfpv4|vxe).c",
                x86="sse sse2", ppc64="vsx", armhf="neon neon_fp16",
                aarch64="", ppc64le="", s390x="vx"
            )
            self.expect(o,
                t***_files=".*cpu_(popcnt|vsx3).c",
        ...
        try:
            self.expect("native",
                t***_flags=".*(-march=native|-xHost|/QxHost).*",
                x86=".*", ppc64=".*", armhf=".*", s390x=".*"
            )
            if self.march() != "unknown":
                raise AssertionError(
    ...
    def test_targets(self):
        ...
        self.expect_targets(
            ...
            */
            """,
            baseline="",
            t***_files=".*(avx2|avx512f|vsx3|vsx4|asimddp|vxe2).c",
...
def new_test(arch, cc):
    if is_standalone: return textw***.dedent("""\
