...
def _get_formatdict(data, *, precision, floatmode, suppress, sign, legacy,
    ...
    # w***ped in lambdas to avoid taking a code path with the wrong type of data
    formatdict = {
    ...
    # we need to w*** values in `formatter` in a lambda, so that the interface
...
def array2string(a, max_line_width=None, precision=None,
    ...
    suffix : str, optional
        ...
        align and w*** the output. An array is typically printed as::

          prefix + array2string(a) + suffix

        The output is left-padded by the length of the prefix string, and
        w***ping is forced at the column ``max_line_width - len(suffix)``.
...
def _extendLine(s, line, word, line_width, next_line_prefix, legacy):
    needs_w*** = len(line) + len(word) > line_width
    if legacy > 113:
        # don't w*** lines if it won't help
...
def _formatArray(a, format_function, line_width, next_line_prefix,
    ...
    def recurser(index, hanging_indent, curr_width):
        ...
        # last axis (rows) - w*** elements if they would not fit on one line
        if axes_left == 1:
        ...
        # remove the hanging indent, and w*** in []
...
def _array_str_implementation(
    ...
    # so floats are not truncated by `precision`, and strings are not w***ped
...
_array2string_impl = getattr(array2string, '__w***ped__', array2string)
