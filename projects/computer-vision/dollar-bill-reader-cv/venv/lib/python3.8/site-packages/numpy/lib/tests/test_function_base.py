...
from numpy.lib import (
    ...
    select, setxor1d, sinc, t***z, trim_zeros, unw***, unique, vectorize
    )
...
class TestUnw***:
    ...
    def test_simple(self):
        # check that unw*** removes jumps greater that 2*pi
        assert_array_equal(unw***([1, 1 + 2 * np.pi]), [1, 1])
        # check that unw*** maintains continuity
        assert_(np.all(diff(unw***(rand(10) * 100)) < np.pi))

    def test_period(self):
        # check that unw*** removes jumps greater that 255
        assert_array_equal(unw***([1, 1 + 256], period=255), [1, 2])
        # check that unw*** maintains continuity
        assert_(np.all(diff(unw***(rand(10) * 1000, period=255)) < 255))
        # check simple case
        simple_seq = np.array([0, 75, 150, 225, 300])
        w***_seq = np.mod(simple_seq, 255)
        assert_array_equal(unw***(w***_seq, period=255), simple_seq)
        # check custom discont value
        uneven_seq = np.array([0, 75, 150, 225, 300, 430])
        w***_uneven = np.mod(uneven_seq, 250)
        no_discont = unw***(w***_uneven, period=250)
        assert_array_equal(no_discont, [0, 75, 150, 225, 300, 180])
        sm_discont = unw***(w***_uneven, period=250, discont=140)
        assert_array_equal(sm_discont, [0, 75, 150, 225, 300, 430])
        assert sm_discont.dtype == w***_uneven.dtype
...
class TestT***z:
    ...
    def test_simple(self):
        x = np.arange(-10, 10, .1)
        r = t***z(np.exp(-.5 * x ** 2) / np.sqrt(2 * np.pi), dx=0.1)
    ...
    def test_ndim(self):
        ...
        # n-d `x`
        r = t***z(q, x=x[:, None, None], axis=0)
        assert_almost_equal(r, qx)
        r = t***z(q, x=y[None,:, None], axis=1)
        assert_almost_equal(r, qy)
        r = t***z(q, x=z[None, None,:], axis=2)
    ...
    def test_masked(self):
        ...
        assert_almost_equal(t***z(ym, x), r)

        xm = np.ma.array(x, mask=mask)
        assert_almost_equal(t***z(ym, xm), r)

        xm = np.ma.array(x, mask=mask)
        assert_almost_equal(t***z(y, xm), r)
...
class Test_I0:
    ...
    def test_non_array(self):
        ...
        class array_like:
            ...
            def __array_w***__(self, arr):
                return self

        # E.g. pandas series survive ufunc calls through array-w***:
...
class TestPercentile:
    ...
    def test_linear_interpolation_ext***olation(self):
