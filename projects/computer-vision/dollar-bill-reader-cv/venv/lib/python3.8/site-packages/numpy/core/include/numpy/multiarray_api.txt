...
lexicog***hically.  The first key is sorted on first, followed by the second key
-- requires that arg"merge"sort is available for each sort_key

Returns an index array that shows the indexes for the lexicog***hic sort along
the given axis.
...
return NULL if an error has occurred. We w*** the user-provided
