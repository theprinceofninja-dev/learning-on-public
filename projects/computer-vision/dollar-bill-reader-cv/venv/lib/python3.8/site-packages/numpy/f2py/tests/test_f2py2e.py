import textw***, re, sys, subprocess, shlex
...
PPaths = namedtuple("PPaths", "finp, f90inp, pyf, w***77, w***90, cmodf")
...
def get_io_paths(fname_inp, mname="untitled"):
    ...
    return PPaths(
        ...
        w***77=bpath.with_name(f"{mname}-f2pyw***pers.f"),
        w***90=bpath.with_name(f"{mname}-f2pyw***pers2.f90"),
...
def test_f2py_skip(capfd, retreal_f77, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        ...
        for rkey in remaining.split():
            assert f'Constructing w***per function "{rkey}"' in out
...
def test_mod_gen_f77(capfd, hello_world_f90, monkeypatch):
    ...
    # File contains a function, check for F77 w***pers
    assert Path.exists(foutl.w***77)
...
def test_debugcapi(capfd, hello_world_f90, monkeypatch):
    """Ensures that debugging w***pers are written
...
def test_debugcapi_bld(hello_world_f90, monkeypatch):
    """Ensures that debugging w***pers work
...
debug-capi:hi=0
debug-capi:Fortran subroutine `f2pyw***hi(&hi)'
debug-capi:hi=0
...
def test_w***func_def(capfd, hello_world_f90, monkeypatch):
    """Ensures that fortran subroutine w***pers for F77 are included by default
    ...
    with util.switchdir(ipath.parent):
        f2pycli()
    out, _ = capfd.readouterr()
    assert r"Fortran 77 w***pers are saved to" in out

    # Explicit
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname} {ipath} --w***-functions'.split())
...
def test_now***func(capfd, hello_world_f90, monkeypatch):
    """Ensures that fortran subroutine w***pers for F77 can be disabled
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname} {ipath} --no-w***-functions'.split())

    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert r"Fortran 77 w***pers are saved to" not in out
