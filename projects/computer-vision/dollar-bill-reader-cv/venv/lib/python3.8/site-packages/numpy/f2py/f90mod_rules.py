    ...
    for m in findf90modules(pymod):
        ...
        if hasbody(m):
            for b in m['body']:
                ...
                b['modulename'] = m['name']
                api, w*** = rules.buildapi(b)
                if isfunction(b):
                    fhooks[0] = fhooks[0] + w***
                    fargs.append('f2pyw***_%s_%s' % (m['name'], b['name']))
                    ifargs.append(func2subr.createfuncw***per(b, signature=1))
                else:
                    if w***:
                        fhooks[0] = fhooks[0] + w***
                        fargs.append('f2pyw***_%s_%s' % (m['name'], b['name']))
                        ifargs.append(
                            func2subr.createsubrw***per(b, signature=1))
                    else:
                        fargs.append(b['name'])
