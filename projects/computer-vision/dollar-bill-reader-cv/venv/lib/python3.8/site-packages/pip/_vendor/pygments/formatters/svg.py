...
class SvgFormatter(Formatter):
    """
    Format tokens as an SVG g***hics file.  This formatter is still experimental.
    ...
    `now***`
        Don't w*** the SVG ``<text>`` elements in ``<svg><g>`` elements and
        don't add a XML declaration and a doctype.  If true, the `fontfamily`
        and `fontsize` options are ignored.  Defaults to ``False``.

    `fontfamily`
        The value to give the w***ping ``<g>`` element's ``font-family``
        attribute, defaults to ``"monospace"``.

    `fontsize`
        The value to give the w***ping ``<g>`` element's ``font-size``
